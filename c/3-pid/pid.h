...
struct pid_settings_t {
  ...
  /// When ADC reads below this signal, the state is set to `close` to the motor
  /// magnet. If the state has changed, a new revolution is counted.
  uint8_t revolution_treshold_close;
  /// When ADC reads above this signal, the state is set to `far` from the motor
  /// magnet.
  uint8_t revolution_treshold_far;
  /// Revolutions are binned in a ring buffer based on when they happened. More
  /// recent revolutions are in the tail of the buffer, while old ones are in
  /// the head of the buffer (soon to be replaced).
  ///
