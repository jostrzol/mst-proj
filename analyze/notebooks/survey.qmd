...
from analyze.lib.constants import ANALYZE_OUT_DIR, PLOT_DIR
...
OUT_SURVEY_PATH = ANALYZE_OUT_DIR / "survey-cleaned.csv"
...
class Column(Enum):
    ...
    S2_LANGUAGES_USED = """Which programming languages do you currently use or
        have used in the past for microcontroller projects?"""
    ...
    S5_DID_USE_MODERN = """Have you ever used any “modern” language (e.g. Rust, Zig)
    ...
    S6_BUILD_SYSTEM_SATISFACTION = """Rate your satisfaction with the build
        system for C/C++ that you use for embedded projects."""
    S6_STATIC_ANALYSIS_RELIANCE = """How much do you rely on C/C++ static
        analysis tools for microcontroller code? (e.g. clang-tidy, MISRA checkers)"""
    S6_STATIC_ANALYSIS_SATISFACTION = """Rate your satisfaction with C/C++
        static analysis tools for microcontroller code."""

    # Section 7: Open section
    S7_WANTED_FEATURES = """Can you think of any language or tooling features
        would most help your embedded development?"""
...
def pd_plot_hist_diff(
    series_total: pd.Series,
    series_overlay: list[pd.Series],
    ax: Axes | None = None,
    xrange: Iterable[Any] | None = None,
    rotation: bool | float = False,
    ymax: float | None = None,
    ...
    total = len(series_total)

    def prepare_values(series: pd.Series) -> pd.Series:
    ...
    values_total = prepare_values(series_total)

    ys_total = values_total / total * 100

    [*colors_overlay, black] = gray_shades(len(series_overlay) + 1, start=0.8)

    ax = ax or plt.axes()
    bar = plot_bar(
        xs=values_total.index.to_list(),
        ys=ys_total.to_list(),
        barlabels=False,
        ax=ax,
        rotation=rotation,
    ...
    width_overlay = (width - padding) / len(series_overlay)
    single_padding = padding / (len(series_overlay) + 1)
    offset_init = -width / 2 + single_padding + width_overlay / 2
    for i, (series, color) in enumerate(zip(series_overlay, colors_overlay)):
        values_overlay = prepare_values(series)
        ys_overlay = values_overlay / total * 100
        offset = offset_init + i * (width_overlay + single_padding)
        bar = plot_bar(
            xs=np.arange(len(values_total.index)) + 1 + offset,
            ys=ys_overlay.to_list(),
            barlabels=False,
            ax=ax,
            rotation=rotation,
            ymax=False,
            colors=color,
            widths=width_overlay,
            xticks=False,
        )
...
def pd_plot_pie(
    ...
    colors = gray_shades(len(values), end=0.6)
...
def pd_savefig(series: pd.Series | str, dir: Path | str):
...
values = preprocess(
    ...
    replace={
        ...
        "Stage lighting equipment": "Specialized electronics",
        "Professionnel Photography/Lighting": "Specialized electronics",
        "test and measurements": "Specialized electronics",
        "Metering": "Specialized electronics",
        "Datacenter": "Specialized electronics",
        "Hobby": "IoT / Edge Computing",
...
values = preprocess(
    ...
    replace={
        ...
        "Aerospace": "Lotnictwo",
        "Specialized electronics": "Elektronika specjalizowana",
...
plt.figure(figsize=figsize_rel(w=0.60))
...
plt.figure(figsize=figsize_rel(w=0.35))
...
frequencies = df[columns].apply(pd.Series.value_counts)
ymax = np.max(frequencies, axis=None)
...
for column in columns:
    ...
    avg = to_plot.mean()
    xlabel = "\n".join(wrap(f"{column.name} ({avg:.2f} avg)", 22))
...
### Industry:IoT vs Industry:Customer Electronics

#### IoT vs CE: Section 3: Pain points and challanges

```{python}
# | layout-ncol: 3

df_iot = df_out[df_out[Column.S1_INDUSTRY].map(lambda row: "Internet rzeczy" in row)]
df_ce = df_out[
    df_out[Column.S1_INDUSTRY].map(
        lambda row: "Elektronika dla masowego klienta" in row
    )
]
...
frequencies = df_out[columns].apply(pd.Series.value_counts)
ymax = np.max(frequencies, axis=None)
...
for column in columns:
    ...
    pd_plot_hist_diff(
        df_out[column],
        [df_iot[column], df_ce[column]],
        ax=ax,
        xrange=range(1, 6),
        ymax=ymax,
    )
    pd_savefig(f"{column.name}-iot-vs-ce", OUT_DIR)

    plt.show()
```

#### IoT vs CE: Section 4: Language selection criteria
...
for column in columns:
    ...
    plt.show()
```
* mamy wymieszaną grupę ludzi bardzo i średnio doświadcznia
* podatności niewidoczne u doświadczonych
* problemy z pamięcią wciąż są istotne na prog sys wbud
* wykresy dla IOT, który charakteryzuje się produkcją małych, ograniczonych, szytych na miarę urządzeń
* pamięć niewielka, możliwości ograniczone
* zarządzanie pamięcią ważniejsze, reszta wykresów podobna
* po wykresie mocy widać, że to IOT (więc ograniczona pamięć, itp)

### No memory safety

#### No mem. safety: Industry segment

```{python}

df_iot = df_out[df_out[Column.S3_MEMORY_SAFETY] != 1]

column = Column.S1_INDUSTRY
values = df_iot[column]
