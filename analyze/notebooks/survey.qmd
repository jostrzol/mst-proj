...
#### IoT: Section 3: Pain points and challanges

```{python}
# | layout-ncol: 3

df_filtered = df_out[
    df_out[Column.S1_INDUSTRY].map(lambda row: "Internet rzeczy" in row)
]
...
frequencies = df_filtered[columns].apply(pd.Series.value_counts)
ymax = np.max(frequencies, axis=None)
...
for column in columns:
    ...
    avg = to_plot.mean()
    xlabel = "\n".join(wrap(f"{column.name} ({avg:.3} avg)", 22))
    # ax.set_xlabel(xlabel)
    plt.show()
```

#### IoT Section 4: Language selection criteria
...
#### Consumer elec.: Section 3: Pain points and challenges
...
df_filtered = df_out[
    df_out[Column.S1_INDUSTRY].map(
        lambda row: "Elektronika dla masowego klienta" not in row
    )
]
...
#### Consumer elec: Section 4: Language selection criteria
...
df_filtered = df_out[df_out[Column.S3_MEMORY_SAFETY] != 1]
