...
def plot_hist(
    ...
    ax.bar_label(bar, values)

    ax.margins(y=0.2, x=0.5 / len(ys))
    ax.yaxis.set_major_formatter(PercentFormatter(decimals=0))

    if save and series.name is not None:
...
def plot_hist_horizontal(
    ...
    fontsize: int | None = None,
    xlabel_side: Literal["left"] | Literal["right"] = "left",
) -> Axes:
    values = series.value_counts()
    ...
    ax.set_yticks(ys, values.index)
    ax.tick_params(axis="y", length=0)
    ax.tick_params(axis="y", which="minor", length=0)

    if xlabel_side == "right":
        ax.tick_params(labelleft=False, labelright=True)
        ax.invert_xaxis()

    ax.bar_label(bar, values, padding=3)

    ax.margins(x=0.2, y=0.8 / len(ys))
...
plot_hist_horizontal(to_plot, ylabel="")
plt.show()
```
...
to_plot = values.explode().str.split().apply("\n".join)
plot_hist(to_plot, rotation=True)
plt.show()
```
...
plot_hist(values.explode(), rotation=True)
plt.show()
```
...
to_plot = values.explode()
plot_hist_horizontal(to_plot, xlabel_side="right")
plt.show()
```
...
plt.figure(figsize=figsize_rel(0.3, 0.6))
plot_hist(values, sort_by_index=True, xrange=range(1, 6))
plt.show()
```
...
plot_hist(to_plot, sort_by_index=True, xrange=range(0, 6))
plt.show()
```
...
plt.figure(figsize=figsize_rel(0.3, 0.6))
to_plot = values[~values.isna()].astype(np.int8)
plot_hist(to_plot, sort_by_index=True, xrange=range(1, 6), save=False)
plt.show()
```
...
plot_hist_horizontal(tags.explode())
```
