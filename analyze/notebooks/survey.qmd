...
import pandas as pd
import numpy as np
import scienceplots  # pyright: ignore[reportMissingTypeStubs]
...
def plot_hist(
    ...
    # TODO: dodać linie z xticks do barów
    ...
    ax = values.plot(kind="bar", xlabel="")
    ...
    ax.set_ylim(top=max(values) * (1 + ymargin))
    ...
    ax.set_xlabel(xlabel or "", fontsize=xlabel_fontsize)
...
to_plot = wrap_series(values.explode(), sep="\n...")
plot_hist(to_plot, rotation=True, xlabel="", xtick_fontsize=14)
plt.show()
```
...
values = preprocess(
    values,
    replace={
        "Software engineer": "Inżnier oprogramowana",
...
to_plot = wrap_series(values.explode(), count=20)
plot_hist(to_plot, xtick_fontsize=15)
plt.show()
```
...
df_out[column] = values
plot_hist(values.explode(), rotation=True, xmargin=0.02)
plt.show()
```
...
values = preprocess(
    values,
    replace={
        "Difficulty of integrating into existing codebase": "Trudność integracji z istniejącym kodem",
        "Team unfamiliarity": "Brak ekspertyzy zespołu",
        "Lack of mature toolchain": "Brak dojrzałości",
...
to_plot = wrap_series(values.explode(), count=24, sep="\n...")
plot_hist(to_plot, rotation=True, xtick_fontsize=14)
plt.show()
```
...
plot_hist(to_plot, sort_by_index=True, range=range(1, 6))
plt.show()
```
