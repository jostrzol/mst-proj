    ...
    TAdcPin: ADCPin<Adc = TAdc>,
{
        ...
        TTimer: Timer,
    {
        ...
        let readings =
            AllocRingBuffer::<f32>::new(options.time_window_bins * options.reads_per_bin as usize);
    ...
    pub fn control_phase(&mut self) -> anyhow::Result<()> {
        ...
        let reading_min = readings.iter().fold(f32::INFINITY, |a, &b| a.min(b));
        let reading_max = readings.iter().fold(0f32, |a, &b| a.max(b));

        use InputRegister::*;
        self.registers.as_mut().write_input(Frequency, frequency);
        self.registers
            .as_mut()
            .write_input(ControlSignal, control_signal);
        self.registers.as_mut().write_input(ValueMin, reading_min);
        self.registers.as_mut().write_input(ValueMax, reading_max);

        info!("readings range: [{:.2}, {:.2}]", reading_min, reading_max);
